...
document.querySelector("[method='GET']").addEventListener('submit',(e) => {
    e.preventDefault();
fetch('http://127.0.0.1/webservice/words',{ method : 'GET'}).then( (response) => {
...
fetch('http://127.0.0.1/webservice/words', { method : 'POST', body : formData}).then( (response) => {
...
fetch('http://127.0.0.1/webservice/words', { method : 'DELETE'}).then((response) => {
...
fetch('http://127.0.0.1/webservice/words', { method : 'PUT'}).then( (response) => {
    console.log(response);
return response.text();
}).then((data) => {
    console.log(data);
